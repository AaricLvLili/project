class MineRevengeWin extends BaseEuiPanel {
	...
	open() {
		this.m_bg.init(`MineRevengeWin`, GlobalConfig.jifengTiaoyueLg.st100560)
	...
	refushInfo() {
		this.nametx.text = GlobalConfig.jifengTiaoyueLg.st101730 + this.data.roberName, this.AttrNum.text = GlobalConfig.jifengTiaoyueLg.st100809 + this.data.fightPow, this.time.text = GlobalConfig.jifengTiaoyueLg.st101731 + DateUtils.GetFormatSecond(this.data.robTime, 8), this.reward.dataProvider = new eui.ArrayCollection(MineModel.countRewardList(this.data.mineType, !0, 1)), ErrorLog.Assert(this.data, "MineRevengeWin   this.data") || (this.handImg.source = "propIcon_048_png")
	}
	...
	onTap(e) {
		switch (e.target) {
			case this.attBtn:
				MineModel.ins().breakRobOther(this.data.robIndex), GameGlobal.mineModel.showType = this.data.mineType, ViewManager.ins().close(MineRevengeWin)
		}
	}
}
window["MineRevengeWin"]=MineRevengeWin