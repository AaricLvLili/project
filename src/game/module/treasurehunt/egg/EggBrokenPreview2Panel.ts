class EggBrokenPreview2Panel extends BaseEuiPanel {
public static LAYER_LEVEL = LayerManager.UI_Popup
	...
	open(...param: any[]) {
		...
		this.group.dataProvider = new eui.ArrayCollection(RewardData.ToRewardDatas(GlobalConfig.ins("SEtotalrewardsConfig")[this.m_Index].box))
	}
	...
	private _OnClick() {
		if (EggBroken.ins().count >=GlobalConfig.ins("SEtotalrewardsConfig")[this.m_Index].time) {
			EggBroken.ins().sendHitting(this.m_Index);
		} else {
...
window["EggBrokenPreview2Panel"]=EggBrokenPreview2Panel